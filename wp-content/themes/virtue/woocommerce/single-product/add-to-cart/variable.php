...
global $woocommerce, $product, $post, $virtue;
?>
			...
			<?php $loop = 0; foreach ( $attributes as $name => $options ) : $loop++; ?>
				<tr>
					...
					<td class="product_value"><select id="<?php echo esc_attr( sanitize_title($name) ); ?>" name="attribute_<?php echo sanitize_title($name); ?>" class="kad-select">
						<option value=""><?php echo __( 'Choose an option', 'virtue' ) ?>&hellip;</option>
						<?php
							if ( is_array( $options ) ) {
						...
						if ( sizeof($attributes) == $loop )
							echo '<div style="display:block"><a class="reset_variations" href="#reset">' . __( 'Clear selection', 'virtue' ) . '</a><div>';
					?></td>
				</tr>
	        <?php endforeach;?>
		</tbody>
